 ...
 */
	...
	PRIVILEGED_DATA static signed char * volatile pcTraceBuffer;
		...
		{																							\
			...
			{																						\
				...
				{																					\
					...
					memcpy(pcTraceBuffer, (unsigned long *) &xTickCount, sizeof( unsigned long));	\
					pcTraceBuffer += sizeof( unsigned long );										\
					memcpy(pcTraceBuffer, &uxPreviousTask, sizeof( unsigned long));					\
					pcTraceBuffer += sizeof( unsigned long );										\
				}																					\
				else																				\
				{																					\
...
tskTCB * pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		#else
		{
			...
			configASSERT( ( ( ( uintptr_t ) pxNewTCB->pxStack & ( uintptr_t ) portBYTE_ALIGNMENT_MASK ) == 0UL ) );
...
void vTaskStartScheduler( uint32_t fcpu, int idle_task_stack_size )
{
portBASE_TYPE xReturn;

	/* Add the idle task at the lowest priority. */
	xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE", idle_task_stack_size, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), ( xTaskHandle * ) NULL );
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				else
				{
					...
					if( ulStatsAsPercentage > 0UL )
					{
						...
						{
							sprintf( pcStatsString, "%s\t\t%lu\t\t%lu%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter, ulStatsAsPercentage );
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							printf() library can be used. */
							sprintf( pcStatsString, "%s\t\t%u\t\t%u%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage );
						}
						#endif
					}
					else
					{
						...
						#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
						{
							sprintf( pcStatsString, "%s\t\t%lu\t\t<1%%\r\n", pxNextTCB->pcTaskName, pxNextTCB->ulRunTimeCounter );
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							printf() library can be used. */
							sprintf( pcStatsString, "%s\t\t%u\t\t<1%%\r\n", pxNextTCB->pcTaskName, ( unsigned int ) pxNextTCB->ulRunTimeCounter );
						}
						#endif
					}
				}
